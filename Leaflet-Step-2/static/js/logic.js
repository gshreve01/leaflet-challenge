...
function getPointsFromCoordinates(featureCoordinates) {
    var coordinates = [];
    if (featureCoordinates.length == 1) {
        coordinates = getPointsFromCoordinates(featureCoordinates[0]);
    }
    else {
...
// second data set on tectonic plates
d3.json("https://raw.githubusercontent.com/fraxen/tectonicplates/master/GeoJSON/PB2002_plates.json", function (tectonicData) {
    console.log("tectonicData", tectonicData);
    ...
    function onEachFeature(feature, layer) {
        ...
        var coordinates = getPointsFromCoordinates(featureCoordinates);
        // console.log("coordinates", coordinates);
        if (coordinates.length == featureCoordinates.length) {
            var line = { "type": "LineString", "coordinates": coordinates };
            ...
            L.geoJSON(myLines, {
                style: myStyle
            }).bindPopup("<h3>" + feature.properties.PlateName + "</h3>")
                .addTo(layers.PLATES);
        }
        else {
            console.log("something went wrong.....coordinate counts are different");
        }
    }
